🌟【新增】优先级（四象限）：待办和共享待办支持 p1-p4 优先级选择，含旧数据兜底
🌟【新增】add-todo 页四象限选择器（P1 紧急重要/ P2 重要不紧急/ P3 紧急不重要/ P4 不紧急不重要）
🌟【新增】todo 列表左侧优先级色条（CSS ::before），todo-detail 页展示优先级
✨【优化】移除重复的 LICENSE.txt 文件
